...
public class SpaceInvadersObject implements Constants
{
	...
	{
		_type = type;
		_image = new Image(resourcesPath + effectsPath + type);
		_moveDirection = moveDirection;
		_speed = speed;
		_location = new Point(x, y);
	}

	public SpaceInvadersObject(final String type, final int moveDirection, final int speed, final Point point)
	{
		_type = type;
		_moveDirection = moveDirection;
		_speed = speed;
	...
	{
		g.texture(new Rectangle(_location.getX(), _location.getY(), 8, 8), _image);
	}
	...
	public void step()
	{
		final double angle = Math.toRadians(_moveDirection);
